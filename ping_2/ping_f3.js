...
async function pingRange(startIp, endIp) {
    ...
    results.forEach(result => {
        if (result.alive) {
            console.log('\x1b[38;5;214m%s\x1b[0m', `Host ${result.ip} está online!`); // Laranja claro
        } else {
            console.log('\x1b[38;5;250m%s\x1b[0m', `Host ${result.ip} não foi alcançado.`); // Cinza
        }
    });
...
const startIp0 = '192.168.3.0';
const endIp0 = '192.168.3.255';
